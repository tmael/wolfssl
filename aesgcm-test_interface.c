...
int main(void)
{
    ...
    (void)auiElapsed;
    printf("Entering aesgcm-test_interface.c \n");
#endif
    for (int i = gcm_vectors_count-1; i > gcm_vectors_count - 52; i--)
    {
        ...
        // printf("iv_len=%d aad_len=%d pt_len=%d tag_len=%d\r\n",
        //            tv->iv_len, tv->aad_len, tv->pt_len, tv->tag_len);

        // printf("\r\nPT:");
        // printHexArray(tv->pt,tv->pt_len);
        // printf("\r\nCT:");
        // printHexArray(tv->ct,tv->ct_len);

        // printf("\r\nAddress of tv->pt:%p",tv->pt);
        // printf("\r\nAddress of tv->ct:%p",tv->ct);
        // printf("\r\nAddress of tv->tag:%p",tv->tag);

        // memset(plaintext, 0, MAX_CT_LEN);
        // memset(ciphertext, 0, MAX_CT_LEN);
        // memset(obs_tag, 0, MAX_CT_LEN);
        /* Key Schedule Setting */
