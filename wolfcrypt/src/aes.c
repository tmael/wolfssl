...
/*!
    ...
    for use with async hardware.

    \return 0 Success
    \return BAD_FUNC_ARG Returned if aes key is null
...
/*!
    \ingroup AES
    \brief Allows retrieval of the key size from the AES structure.

    \return 0 On successfully getting the key
    \return BAD_FUNC_ARG Returned if key length is invalid

    \param aes pointer to a AES object
    \param keySize pointer to the output buffer in which to store the key size
